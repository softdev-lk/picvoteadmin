/*
 * picvote ADMIN TEMPLATE JAVASCRIPT
 * ======================================================================
...
For more efficient, simply called "picvote.container".
...
!function ($) {
	...
	picvote.window.on('load', function(){
		...
		picvote.body.addClass('picvote-ready');
	});
...
!function ($) {
	...
	picvote.window.one('load', function(){
		if (picvote.scrollTop.length && !picvote.isMobile) {
			var isVisible = true;
			var offsetTop = 250;

			picvote.window.scroll(function(){
				if (picvote.window.scrollTop() > offsetTop && !isVisible) {
					picvote.navbar.addClass('shadow');
					picvote.scrollTop.addClass('in');
					isVisible = true;
				}else if (picvote.window.scrollTop() < offsetTop && isVisible) {
					picvote.navbar.removeClass('shadow');
					picvote.scrollTop.removeClass('in');
					isVisible = false;
				}
			});

			picvote.scrollTop.on('click', function(e){
				e.preventDefault();

				picvote.bodyHtml.animate({
					scrollTop : 0
				}, 500);
			});
...
!function ($) {
	...
	methods = {
		'show' : function(el){
			...
			ovId = 'picvote-overlay-' + uID() + uID()+"-" + uID(),
			panelOv = $('<div id="'+ ovId +'" class="panel-overlay"></div>');

			el.prop('disabled', true).data('picvoteOverlay',ovId);
		...
		'hide': function(el){
			var target = $(el.attr('data-target'));
			var boxLoad = $('#'+ el.data('picvoteOverlay'));
	...
	$.fn.picvoteOverlay = function(method){
		if (methods[method]){
...
!function ($) {
	...
	$.picvotenoty = function(options){
		...
		focusElement = function(pos){
			picvote.bodyHtml.animate({scrollTop: pos}, 300, function(){
				el.addClass('in');
			});
		},
		init = function(){
			if (opt.container === 'page') {
				if (!pageHolder) {
					...
					picvote.contentContainer.prepend(pageHolder);
				};
				...
				if (!floatContainer[opt.floating.position]) {
					...
					picvote.container.append(floatContainer[opt.floating.position]);
				}
...
!function ($) {
	...
	formElement = function(el){
		if (el.data('picvote-check')) return
		else
		el.data('picvote-check', true)
		...
		changed = function(){
			if(input.type == 'radio' && $groupInput.length) {
				$groupInput.each(function(){
					var $gi = $(this);
					if ($gi.hasClass('active')) $gi.trigger('picvote.ch.unchecked');
			...
			if (input.checked) {
				el.addClass('active').trigger('picvote.ch.checked');
			}else{
				el.removeClass('active').trigger('picvote.ch.unchecked');
			}
		};
	...
	$.fn.picvotecheck = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
	...
	picvote.document.ready(function() {
		allFormEl = $('.form-checkbox, .form-radio');
		if(allFormEl.length) allFormEl.picvotecheck();
	});

	picvote.document.on('change', '.btn-file :file', function() {
		var input = $(this),
...
!function ($) {
		...
		$menulink.each(function(){
			...
			on('shown.bs.popover', function () {
				...
				offsetBottom 	= function(){
					if(picvote.container.hasClass('mainnav-fixed')){
				...
				if( offsetTop > offsetBottom){
					...
					if(!picvote.container.hasClass('navbar-fixed') && picvote.mainNav.hasClass('affix-top')) offsetBottom -= 50;
					if(popoverHeight > offsetBottom){
						if(picvote.container.hasClass('navbar-fixed') || picvote.mainNav.hasClass('affix-top')) offsetBottom -= (elHeight + 5);
			...
			.on('click', function(){
				if(!picvote.container.hasClass('mainnav-sm')) return;
	...
	updateSize = function(){
		...
		var sw = picvote.container.width(), currentScreen;
		...
		if (screenCat != currentScreen){
			...
			picvote.screenSize = currentScreen;

			if(picvote.screenSize == 'sm' && picvote.container.hasClass('mainnav-lg')){
				$.picvoteNav('collapse');
			}
		}
	},
	updateNav = function(e){
		...
		if (updateMethod == 'collapse' || picvote.container.hasClass('mainnav-sm') ) {
			picvote.container.removeClass('mainnav-in mainnav-out mainnav-lg');
			bindSmallNav();
		}
	...
	init = function(){
		if (!defaultSize) {
			defaultSize = {
				xs : 'mainnav-out',
				sm : picvote.mainNav.data('sm') || picvote.mainNav.data('all'),
				md : picvote.mainNav.data('md') || picvote.mainNav.data('all'),
				lg : picvote.mainNav.data('lg') || picvote.mainNav.data('all')
			}
	...
	methods = {
		'revealToggle' : function(){
			if (!picvote.container.hasClass('reveal')) picvote.container.addClass('reveal');
			picvote.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealIn' : function(){
			if (!picvote.container.hasClass('reveal')) picvote.container.addClass('reveal');
			picvote.container.addClass('mainnav-in').removeClass('mainnav-out mainnav-lg mainnav-sm');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealOut' : function(){
			if (!picvote.container.hasClass('reveal')) picvote.container.addClass('reveal');
			picvote.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'slideToggle' : function(){
			if (!picvote.container.hasClass('slide')) picvote.container.addClass('slide');
			picvote.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		...
		'pushToggle' : function(){
			picvote.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm');
			if (picvote.container.hasClass('mainnav-in mainnav-out')) picvote.container.removeClass('mainnav-in');
			//if (picvote.container.hasClass('mainnav-in')) //picvote.container.removeClass('aside-in');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'pushIn' : function(){
		...
		'colExpToggle' : function(){
			if (picvote.container.hasClass('mainnav-lg mainnav-sm')) picvote.container.removeClass('mainnav-lg');
			picvote.container.toggleClass('mainnav-lg mainnav-sm').removeClass('mainnav-in mainnav-out');
			return picvote.window.trigger('resize');
		},
		'collapse' : function(){
			picvote.container.addClass('mainnav-sm').removeClass('mainnav-lg mainnav-in mainnav-out');
			updateMethod = 'collapse';
			return picvote.window.trigger('resize');
		},
		'expand' : function(){
			picvote.container.removeClass('mainnav-sm mainnav-in mainnav-out').addClass('mainnav-lg');
			return picvote.window.trigger('resize');
		},
		'togglePosition' : function(){
			picvote.container.toggleClass('mainnav-fixed');
			picvote.mainNav.picvoteAffix('update');
		},
		'fixedPosition' : function(){
			picvote.container.addClass('mainnav-fixed');
			picvote.mainNav.picvoteAffix('update');
		},
		'staticPosition' : function(){
			picvote.container.removeClass('mainnav-fixed');
			picvote.mainNav.picvoteAffix('update');
		},
		'update' : updateNav,
	...
	$.picvoteNav = function(method,complete){
		if (methods[method]){
			if(method == 'colExpToggle' || method == 'expand' || method == 'collapse'){
				if(picvote.screenSize == 'xs' && method == 'collapse'){
					method = 'pushOut';
				}else if((picvote.screenSize == 'xs' || picvote.screenSize == 'sm') && (method=='colExpToggle' || method == 'expand') && picvote.container.hasClass('mainnav-sm')){
					method = 'pushIn';
				}
			}
	...
	$.fn.isOnScreen = function(){
		var viewport = {
			top : picvote.window.scrollTop(),
			left : picvote.window.scrollLeft()
		};
		viewport.right = viewport.left + picvote.window.width();
		viewport.bottom = viewport.top + picvote.window.height();
	...
	picvote.window.on('resizeEnd',updateNav).trigger('resize');
	...
	picvote.window.on('load', function() {
		...
		if(toggleBtn.length){
			toggleBtn.on('click', function(e){
				...
				if(toggleBtn.hasClass('push')){
				 	 $.picvoteNav('pushToggle');
				}else if(toggleBtn.hasClass('slide')){
					$.picvoteNav('slideToggle');
				}else if(toggleBtn.hasClass('reveal')){
					$.picvoteNav('revealToggle');
				}else{
		...
		if (menu.length) {
			...
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			picvote.mainNav.find('.nano').nanoScroller({
				preventPageScrolling : true
			});
...
!function ($) {
	...
	var asideMethods = {
		'toggleHideShow' : function(){
			picvote.container.toggleClass('aside-in');
			picvote.window.trigger('resize');
			if(picvote.container.hasClass('aside-in')){
				toggleNav();
			}
		},
		'show' : function(){
			picvote.container.addClass('aside-in');
			picvote.window.trigger('resize');
			toggleNav();
		},
		'hide' : function(){
			picvote.container.removeClass('aside-in');
			picvote.window.trigger('resize');
		},
		'toggleAlign' : function(){
			picvote.container.toggleClass('aside-left');
			picvote.aside.picvoteAffix('update');
		},
		'alignLeft' : function(){
			picvote.container.addClass('aside-left');
			picvote.aside.picvoteAffix('update');
		},
		'alignRight' : function(){
			picvote.container.removeClass('aside-left');
			picvote.aside.picvoteAffix('update');
		},
		'togglePosition' : function(){
			picvote.container.toggleClass('aside-fixed');
			picvote.aside.picvoteAffix('update');
		},
		'fixedPosition' : function(){
			picvote.container.addClass('aside-fixed');
			picvote.aside.picvoteAffix('update');
		},
		'staticPosition' : function(){
			picvote.container.removeClass('aside-fixed');
			picvote.aside.picvoteAffix('update');
		},
		'toggleTheme' : function(){
			picvote.container.toggleClass('aside-bright');
		},
		'brightTheme' : function(){
			picvote.container.addClass('aside-bright');
		},
		'darkTheme' : function(){
			picvote.container.removeClass('aside-bright');
		}
	},
	toggleNav = function(){
		if(picvote.container.hasClass('mainnav-in')){
			picvote.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
		}
	};
	...
	$.picvoteAside = function(method,complete){
	...
	picvote.window.on('load', function() {
		if(picvote.aside.length){
			// STYLEABLE SCROLLBARS
			// =================================================================
			// Require nanoScroller
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			picvote.aside.find('.nano').nanoScroller({
				preventPageScrolling : true,
				alwaysVisible: false
			});
...
/* ========================================================================
* picvote LANGUAGE SELECTOR v1.0
* -------------------------------------------------------------------------
...
!function ($) {
	...
	$.fn.picvoteLanguage = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
...
!function ($) {
	...
	$.fn.picvoteAffix = function(method){
		return this.each(function(){
			var el = $(this), className;

			if (typeof method === 'object' || !method){
				className = method.className;
				el.data('picvote.af.class', method.className);
			}else if (method == 'update') {
				className = el.data('picvote.af.class');
			}

			if (picvote.container.hasClass(className) && !picvote.container.hasClass('navbar-fixed') ) {
				el.affix({
					offset:{
					top:$('#navbar').outerHeight()
					}
				});
			}else if (!picvote.container.hasClass(className) || picvote.container.hasClass('navbar-fixed')) {
				picvote.window.off(el.attr('id') +'.affix');
	...
	picvote.window.on('load', function(){
		if (picvote.mainNav.length) {
			picvote.mainNav.picvoteAffix({className : 'mainnav-fixed'});
		}

		if (picvote.aside.length) {
			picvote.aside.picvoteAffix({className : 'aside-fixed'});
		}
	});
